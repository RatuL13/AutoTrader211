...
router.get("/",function(req,res){
  Car.find({},function(err,all_cars){
    if(err)
    console.log(err);
    else
    res.render("car/index",{cars:all_cars,currentUser:req.user});
    });
   });
   ...
   router.post("/user/signup",upload.single('profileimage'),function(req,res){
                 ...
                 if(err)
                 {
                  req.flash("error",err.message);   
                     ...
                     passport.authenticate("local")(req,res,function(){
                      req.flash("success","Welcome to Auto Trader!"+" "+"Nice to meet you"+" "+user.username);   
                      res.redirect("/");
          ...
          User.findById(req.params.id,function(err,user){
            if(err){
              ...
              return res.redirect("back");
                ...
                Car.find().where("author.id").equals(user._id).exec(function(err, cars) {
                    if(err) {
                      req.flash("error", "Something went wrong.");
                      return res.redirect("/");
                    }
                    else
                    res.render("user/profile", {user:user, cars: cars});
                  });
                }
              });
            });
